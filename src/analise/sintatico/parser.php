...
class parser {
    ...
    public function __construct(string $jsonFilePath = null) {
        if ($jsonFilePath === null) {
            $jsonFilePath = __DIR__ . '/../TabelaSintatica/tabelaSintatica.json';
        }
    ...
    private function carregarTabela(string $filePath): void {
        ...
        if ($data === null) {
            throw new \Exception("Arquivo JSON inválido ou mal formatado.");
        }

        if (!isset($data['goto']) || !isset($data['actionTable'])) {
            throw new \Exception("Arquivo JSON inválido: chaves 'goto' ou 'actionTable' ausentes.");
        }
